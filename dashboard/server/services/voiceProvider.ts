...
export type ProviderType = 'vapi' | 'elevenlabs' | 'openai' | 'azure' | 'streamelements' | 'smallestai';
...
export class VoiceProviderFactory {
  ...
  private static createProvider(type: ProviderType): BaseVoiceProvider {
    switch (type) {
      ...
      case 'smallestai':
        // Smallest AI validation with service
        return {
          validateConfig: async () => {
            const { SmallestAIService } = await import('./SmallestAIService');
            const service = new SmallestAIService();
            return await service.validateConfig();
          }
        } as BaseVoiceProvider;
  ...
  static getSupportedProviders(): ProviderType[] {
    return ['vapi', 'streamelements', 'smallestai']; // VAPI, StreamElements, and SmallestAI are implemented
  }
  ...
  static async getProviderInfo(): Promise<ProviderInfo[]> {
    const providers: ProviderInfo[] = [
      ...
      },
      {
        name: 'Smallest AI',
        type: 'smallestai',
        isConfigured: !!process.env.SMALLEST_AI_API_KEY,
        isValidated: false,
      },
    ];
    ...
    for (const provider of providers) {
      if (provider.isConfigured && (provider.type === 'vapi' || provider.type === 'streamelements' || provider.type === 'smallestai')) {
        try {
