...
// Helper function to generate BoostMyLead logo SVG
function generateBoostMyLeadLogoSVG(width = 300, height = 90): string {
...
export class TeamService {
  ...
  static async sendInvite(
    ...
    await sendEmail({
      to: email,
      subject: `Invitation to join ${organization?.name || 'the team'} - BoostMyLead`,
      html: `
        <!DOCTYPE html>
        <html lang="en">
        <head>
          ...
          <title>Team Invitation - BoostMyLead</title>
          <style>
            ...
            .disclaimer { background-color: #f9f9f9; padding: 15px; margin-top: 20px; border-left: 4px solid #F74000; font-size: 11px; color: #555; }
          </style>
        </head>
        <body>
          <div class="header">
            ${generateBoostMyLeadLogoSVG(300, 90)}
          </div>
          
          <div class="content">
            <h2 style="color: #F74000; margin-bottom: 20px;">You've been invited to join our team!</h2>
            <p>Hello,</p>
            <p>${inviter ? `${inviter.firstName} ${inviter.lastName}` : 'A team member'} has invited you to join <strong>${organization?.name || 'our organization'}</strong> as a${role === 'admin' ? 'n' : ''} <strong>${role}</strong> on the BoostMyLead platform.</p>
            <p>BoostMyLead is an advanced AI communication and workflow automation platform that helps teams streamline their operations and boost productivity.</p>
          ...
          <div class="footer">
            ...
            <p>Visit us: <a href="https://boostmylead.xoidlabs.com" style="color: #F74000;">boostmylead.xoidlabs.com</a></p>
          </div>
          
          <div class="disclaimer">
            <p><strong>DISCLAIMER:</strong> This email contains confidential information intended only for the recipient named above. If you are not the intended recipient, you are hereby notified that any disclosure, copying, or distribution of this email is strictly prohibited. The information contained in this email is provided for informational purposes only and does not constitute legal, financial, or professional advice. BoostMyLead reserves the right to monitor email communications for security and compliance purposes. By accepting this invitation, you agree to our Terms of Service and Privacy Policy.</p>
          </div>
        </body>
        </html>
      `
