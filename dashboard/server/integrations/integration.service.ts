...
export class IntegrationService {
   ...
   */
  static async getUserIntegrations(userId: string, organizationId: string): Promise<any[]> {
    try {
  ...
  static async saveIntegrationConfig(
    ...
    switch (type.toUpperCase()) {
      case 'SMTP':
        return await SmtpService.saveConfig(userId, organizationId, name, config);
      case 'HUBSPOT':
        return await HubSpotService.saveConfig(userId, organizationId, name, config);
      case 'ZOHO':
        return await ZohoService.saveConfig(userId, organizationId, name, config);
      case 'WEBHOOK':
        return await WebhookService.saveConfig(userId, organizationId, name, config);
      case 'ELEVENLABS':
        return await ElevenLabsIntegrationService.saveConfig(userId, organizationId, name, config);
      default:
   ...
   */
  static async testIntegrationConfig(type: string, config: any): Promise<boolean> {
    try {
      console.log('IntegrationService: Testing config for type:', type);
      
      switch (type.toUpperCase()) {
        case 'SMTP':
          console.log('IntegrationService: Config details:', { host: config?.host, port: config?.port, secure: config?.secure });
          const smtpResult = await SmtpService.testConfig(config);
          console.log('IntegrationService: SMTP test result:', smtpResult);
          return smtpResult;
        case 'HUBSPOT':
          console.log('IntegrationService: Config details:', { apiKey: config?.apiKey ? '[HIDDEN]' : 'missing' });
          const hubspotResult = await HubSpotService.testConfig(config);
          console.log('IntegrationService: HubSpot test result:', hubspotResult);
        ...
        case 'ZOHO':
          ...
          return zohoResult;
        case 'WEBHOOK':
          console.log('IntegrationService: Config details:', { username: config?.username ? '[HIDDEN]' : 'missing' });
          const webhookResult = await WebhookService.testConfig(config);
          console.log('IntegrationService: Webhook test result:', webhookResult);
        ...
        case 'ELEVENLABS':
          ...
          const elevenLabsResult = await ElevenLabsIntegrationService.testConfig(testConfig);
          console.log('IntegrationService: ElevenLabs test result:', elevenLabsResult);
    ...
    try {
      ...
      switch (integrationConfig.type) {
        case 'SMTP':
          ...
          return await SmtpService.sendEmail(userId, organizationId, configId, emailPayload);
        default:
   ...
   */
  static getSupportedTypes(): Array<{ type: string; name: string; description: string }> {
    return [
      {
        type: 'SMTP',
        name: 'Email (SMTP)',
      ...
      },
      {
        type: 'HUBSPOT',
        name: 'HubSpot CRM',
        description: 'Sync contacts, deals, and companies with your HubSpot account'
      },
      {
        type: 'WEBHOOK',
        name: 'Custom Webhook',
      ...
      },
      {
        ...
        description: 'Clone custom voices for your AI agents using ElevenLabs API'
      }
  ...
  static async getDealByName(
    ...
    try {
      ...
      const integration = IntegrationFactory.create(integrationConfig.type, config);
      ...
      throw new Error(`Failed to get ${integrationConfig?.type || 'CRM'} deal: ${error.message}`);
    }
  }
