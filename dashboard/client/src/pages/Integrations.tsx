...
export default function Integrations() {
  ...
  const [elevenLabsFormData, setElevenLabsFormData] =
    useState<ElevenLabsConfig>({
      apiKey: "",
      voiceName: "",
      voiceDescription: "",
    });
  ...
  const [zohoTokenGeneration, setZohoTokenGeneration] = useState({
    authCode: "",
    redirectUri: window.location.origin + "/integrations", // Default redirect URI
  });
  ...
  const testEmailMutation = useMutation({
    mutationFn: async (data: {
      ...
      return await apiRequest(
        "POST",
        `/api/integrations/${data.configId}/test-email`,
        {
          to: data.to,
  ...
  const getCRMDealMutation = useMutation({
    mutationFn: async (data: { configId: string; dealName: string }) => {
      return await apiRequest(
        "POST",
        `/api/integrations/${data.configId}/get-deal`,
        {
  ...
  // Zoho token generation mutation
  const generateZohoTokensMutation = useMutation({
    mutationFn: async (data: {
      code: string;
      clientId: string;
      clientSecret: string;
      ...
      return await apiRequest("POST", "/api/integrations/zoho/tokens", {
        code: data.code ? `encoded_${btoa(data.code)}` : undefined,
        clientId: data.clientId ? `encoded_${btoa(data.clientId)}` : undefined,
        clientSecret: data.clientSecret
          ? `encoded_${btoa(data.clientSecret)}`
          : undefined,
    ...
    onSuccess: (data) => {
      if (data.data?.refresh_token) {
        ...
        toast({
          title: "Success",
          description: "Zoho refresh token generated and applied!",
        });
      } else {
        toast({
          title: "Error",
          description:
            "Failed to generate refresh token: " + JSON.stringify(data.data),
          variant: "destructive",
        });
      }
    },
  ...
  const handleSave = () => {
    ...
    if (selectedIntegrationType === "SMTP") {
      ...
      configData = {
        ...zohoFormData,
        refreshToken: zohoFormData.refreshToken
          ? `encoded_${btoa(zohoFormData.refreshToken)}`
          : undefined,
        clientId: zohoFormData.clientId
          ? `encoded_${btoa(zohoFormData.clientId)}`
          : undefined,
        clientSecret: zohoFormData.clientSecret
          ? `encoded_${btoa(zohoFormData.clientSecret)}`
          : undefined,
      ...
      <div className="flex-1 p-8 bg-background min-h-screen">
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent className="w-[95vw] max-w-lg mx-auto bg-background border-border max-h-[85vh] flex flex-col">
            ...
            <div className="space-y-4 overflow-y-auto flex-1 px-1 mt-4">
              <div>
                <Label htmlFor="integrationType" className="text-foreground">
                  Integration Type
                </Label>
                <Select
                  ...
                  <div className="flex items-center space-x-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        setSMTPFormData({ ...smtpFormData, secure: checked })
                      }
                      className="data-[state=checked]:bg-[#F74000]"
                    />
                    <Label htmlFor="secure" className="text-foreground">
                      Use SSL/TLS
                    </Label>
                  </div>
                  <div>
                ...
                <>
                  <div>
                    ...
                    <p className="text-muted-foreground text-xs mt-1">
                      Find your API key in HubSpot Settings → Account Setup →
              ...
              {selectedIntegrationType === "ZOHO" && (
                <>
                  ...
                  <div>
                    <Label htmlFor="refreshToken" className="text-foreground">
                      Zoho Refresh Token
                    </Label>
                    <Input
                      id="refreshToken"
                  ...
                  <div>
                    ...
                    <Select
                      value={zohoFormData.region}
                      onValueChange={(value) =>
                        setZohoFormData({ ...zohoFormData, region: value })
                      }
                    >
                      <SelectTrigger className="bg-background border-border text-foreground">
                  ...
                  <div className="bg-accent/30 border border-border rounded-lg p-4 mt-6">
                    ...
                    <p className="text-muted-foreground text-xs mb-4">
                      If you don't have a refresh token, you can generate one
                    ...
                    <div className="space-y-3">
                      <div>
                        ...
                        <Input
                          ...
                          value={zohoTokenGeneration.authCode}
                          onChange={(e) =>
                            setZohoTokenGeneration({
                              ...zohoTokenGeneration,
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          value={zohoTokenGeneration.redirectUri}
                          onChange={(e) =>
                            setZohoTokenGeneration({
                              ...zohoTokenGeneration,
                        ...
                        <p className="text-[10px] text-muted-foreground mt-1">
                          Must match the Redirect URI in Zoho API Console
                        </p>
                      </div>
                      <Button
                        type="button"
                        ...
                        disabled={
                          generateZohoTokensMutation.isPending ||
                          !zohoTokenGeneration.authCode ||
                          !zohoFormData.clientId ||
                          !zohoFormData.clientSecret
                        }
                        onClick={() => {
                          generateZohoTokensMutation.mutate({
                            code: zohoTokenGeneration.authCode,
                            clientId: zohoFormData.clientId || "",
                            clientSecret: zohoFormData.clientSecret || "",
                            redirectUri: zohoTokenGeneration.redirectUri,
                            region: zohoFormData.region || "com",
                          });
                        }}
                      >
                        {generateZohoTokensMutation.isPending
                          ? "Generating..."
                          : "Generate & Apply Refresh Token"}
                      </Button>
                    </div>
                  </div>
                </>
                ...
                <>
                  ...
                  <div className="bg-gray-900/50 border border-gray-700 rounded-lg p-4">
                    ...
                    <p className="text-muted-foreground text-xs mt-2">
                      Use this URL in external platforms like Zapier, Make,
                  ...
                  <div className="bg-blue-900/20 border border-blue-700/30 rounded-lg p-4">
                    ...
                    <ul className="text-blue-200 text-sm space-y-1">
                      <li>
                        • Use HTTP Basic Auth with your username and password
                      </li>
                      <li>
                        • Make GET requests to retrieve call data in JSON format
                      </li>
                      <li>
              ...
              {selectedIntegrationType === "ELEVENLABS" && (
                <>
                  ...
                  <div>
                    <Label htmlFor="voiceName" className="text-foreground">
                      Default Voice Name
                    </Label>
                    <Input
                      id="voiceName"
                  ...
                  <div className="bg-yellow-900/20 border border-yellow-700/30 rounded-lg p-4">
                    ...
                    <ul className="text-yellow-200 text-sm space-y-1">
                      <li>
                        • After setup, you can upload audio files to clone
                        voices
                      </li>
                      <li>
                        • Audio files should be clear and at least 30 seconds
                        long
                      </li>
                      <li>
                        • Cloned voices will appear in your agent TTS settings
                      </li>
                      <li>
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Connect with Zoho CRM for deal management
                  </p>
                </div>
              </div>
              <div className="mt-4">
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Secure webhook endpoint for external platforms
                  </p>
                </div>
              </div>
              <div className="mt-4">
            ...
            <CardContent className="p-6">
              <div className="flex items-center space-x-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Clone custom voices for your AI agents
                  </p>
                </div>
              </div>
              <div className="mt-4">
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center space-x-2 mt-1">
                            <Badge
                              variant="secondary"
                              className="bg-[#F74000]/20 text-[#F74000] border-[#F74000]/30"
                            >
                              {integration.type}
                            </Badge>
        ...
        <Dialog open={testEmailDialog} onOpenChange={setTestEmailDialog}>
          <DialogContent className="bg-black border-gray-800">
            ...
            <div className="space-y-4">
              ...
              <div>
                <Label htmlFor="testSubject" className="text-foreground">
                  Subject
                </Label>
                <Input
                  id="testSubject"
              ...
              <div>
                <Label htmlFor="testMessage" className="text-foreground">
                  Message
                </Label>
                <Textarea
                  id="testMessage"
        ...
        <Dialog open={getDealDialog} onOpenChange={setGetDealDialog}>
          <DialogContent className="bg-black border-gray-800 max-w-2xl">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="dealName" className="text-foreground">
                  Deal Name
                </Label>
                <Input
                  id="dealName"
