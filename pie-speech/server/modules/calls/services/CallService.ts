...
export class CallService {
    ...
    try {
      ...
      const payload = {
        ...
        tts: {
          ...
          model_id: agent.voiceProvider?.toLowerCase() === 'rime' ? 'arcana' : undefined,
          api_key: (() => {
            const provider = agent.voiceProvider?.toLowerCase();
  ...
  async updateCallFromWebhookDemo(callId: string, webhookData: any): Promise<void> {
    ...
    // Find call record and get current webhook array
    const callRecord = await Call.findOne({ twilioSid: callId });
    ...
    try {
      ...
      if (webhookData.type === 'PHONE_CALL_ENDED') {
        ...
        if (webhookData.recording_urls && webhookData.recording_urls.length > 0 && webhookData.recording_urls[0].recording_url) {
          conditionalUpdates.recording = webhookData.recording_urls[0].recording_url;
          console.log(`💰 [Webhook Service] Recording url: ${webhookData.recording_urls[0].recording_url}`);
        }
        
        // Store the cost value from cost_breakdown.total_cost.usd if received
        if (webhookData.cost_breakdown?.total_cost?.usd !== undefined && webhookData.cost_breakdown.total_cost.usd !== null) {
          conditionalUpdates.cost = webhookData.cost_breakdown.total_cost.usd;
          console.log(`💰 [Webhook Service] Recording call cost: $${webhookData.cost_breakdown.total_cost.usd}`);
          
          // Deduct credits from organization
          const callCost = parseFloat(webhookData.cost_breakdown.total_cost.usd);
