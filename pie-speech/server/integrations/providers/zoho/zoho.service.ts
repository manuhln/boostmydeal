...
export class ZohoService {
  ...
  private async getAccessToken(): Promise<string> {
    ...
    console.log('Zoho token response status:', response.status, response.statusText);
    console.log('Zoho token response text (first 500 chars):', responseText.substring(0, 500));
  ...
  static async getConfig(userId: string, organizationId: string, configId?: string): Promise<ZohoConfigData | null> {
    try {
      let query: any = {
        userId,
        organizationId,
  ...
  static async sendTestEmail(
    ...
    try {
      console.log('ZohoService: Test email functionality not implemented for Zoho');
